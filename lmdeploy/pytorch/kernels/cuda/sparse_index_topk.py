...
_CANDIDATE_STATE_SELECTED_BIN = 0
_CANDIDATE_STATE_COUNT_PRIOR = 1
_CANDIDATE_STATE_VALID_COUNT = 2
        ...
        with T.Kernel(num_tokens, threads=threads) as row:
            ...
            while candidate < num_candidates:
                ...
                global_id = T.reinterpret(Candidates[rank, row, slot, 1],
                                          T.int32)
                if global_id >= 0:
                    T.atomic_add(state[_CANDIDATE_STATE_VALID_COUNT], 1)
                    T.atomic_add(rank_counts[rank], 1)
                candidate += threads
            T.sync_threads()

            valid_count = state[_CANDIDATE_STATE_VALID_COUNT]
            if valid_count <= top_k:
                ...
                # an atomic output counter makes identical rows differ.
                if tidx == 0:
                    rank_offsets[0] = 0
                ...
                # Select by the composite key (score, -global_id). This folds
                # stable tie-breaking into the same radix search as scores.
                ...
                for round_idx in T.Unroll(8):
                    ...
                    if tidx < _RADIX_SIZE:
                        ...
                        if (greater_count < selected_rank and
                            ...
                            state[_CANDIDATE_STATE_SELECTED_BIN] = tidx
                            state[_CANDIDATE_STATE_COUNT_PRIOR] = greater_count
                    T.sync_threads()

                    selected_bin = state[_CANDIDATE_STATE_SELECTED_BIN]
                    if round_idx < 4:
                    ...
                    selected_rank -= state[_CANDIDATE_STATE_COUNT_PRIOR]

                # Emit in candidate order. Atomic output allocation can
                # permute an otherwise identical winner set across rows.
